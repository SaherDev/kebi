...
from kebi.core.places import PlaceCore, PlaceSource, UserPlace, UserPlacesService
...
def _make_app(
    ...
    return client, user_places, notes_service
...
def test_unsaved_place_resolves_with_null_user_data() -> None:
    ...
    client, _, _ = _make_app([_core()], save=None)
...
def test_saved_place_carries_user_data_without_user_id() -> None:
    client, _, _ = _make_app([_core()], save=_save())
...
def test_notes_ride_along_with_coarse_source_label() -> None:
    client, _, _ = _make_app([_core()], save=_save(), notes=[_note()])
...
def test_save_ref_is_threaded_only_when_a_save_exists() -> None:
    ...
    client, _, notes_service = _make_app([_core()], save=_save())
...
def test_identity_scopes_the_save_lookup() -> None:
    client, user_places, _ = _make_app([_core()], save=None)
