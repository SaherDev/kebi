...
def _render_movement_context(state: AgentState) -> str:
    ...
    if shape == "corridor" and isinstance(corridor, dict) and corridor.get("stops"):
        ...
        parts.append(
            ...
            "you go, not as a ranked list. You almost certainly know this "
            "route already — name the stops worth stopping at and pass them "
            "to `suggest_places` as `place_names` so they come back verified "
            "as real cards. A famous pass or scenic stretch belongs in the "
            "answer when it is the reason people make the drive: say what to "
            "do there and where to pull over. Never narrate your own reasoning "
            "about what counts as a stop — just write the answer. If a search "
            "comes back route_too_long, the trip "
