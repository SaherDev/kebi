...
**Consequences:** The memory-guide failure mode is gone from observed runs: the model either verifies or says plainly it has no picks there yet, and misfiled saves land in their city. The full target quality showed up when the advanced-model entitlement was on — which also surfaced that every earlier live test had run without entitlement headers, so taste had never loaded; test requests must carry the tier being tested. Two limits. The guard demands verification but cannot make the model comply — one observed run answered honestly-empty rather than call the tool, which is the acceptable failure, and the open gap is conversion, not safety. And the guard keys on candidate coverage, not answer content: a stop covered by one weak candidate passes, so answer quality at covered stops remains the prompt's job.
