...
**Date:** 2026-08-20\
**Status:** accepted — corrects the record ADR-092 assumed it had\
**Context:** The cost story the project told itself was mostly untrue. Eleven of sixteen model roles sent Langfuse spans with no token counts at all: the structured-extraction path threw the raw completion away after parsing it, and the plain-completion path returned bare text, so the spans existed but priced as zero. Retries multiplied invisibly — provider SDKs silently retry twice by default, underneath kebi's own retry loops and underneath the extraction library's validation retries, so one logical call could cost up to nine API calls with a single span to show for it. The orchestrator's prompt caching — the largest single cost lever in the system — was unverifiable, because cache-read and cache-write token counts were explicitly dropped on the way to the trace, meaning cached input was priced at full rate and the hit rate could not be observed at all. A pricing table for token-priced models existed in config but was documentation by its own admission; code never read it. And the docs claimed a Langfuse callback handler and a general LLM-response cache in Redis, neither of which exists. None of this changed behavior, which is why it survived: the answers were right, only the accounting was fiction.\
**Decision:** Telemetry becomes a property of the call, not a courtesy of the call site. Every completion — structured or plain — returns its token usage alongside its payload, so a caller physically cannot get a result without also holding what it cost; the span-ending helper prices that usage from the config pricing table, which is now keyed by model name, cache-aware, current, and read by code, with Langfuse's own catalog demoted to cross-check. Cache-read and cache-write tokens travel as their own buckets and are priced at their own rates, making prompt-cache effectiveness a measurable number for the first time. Retries collapse to a single explicit budget per role in config: SDK-internal retries are disabled everywhere, kebi's loops trace one span per attempt, and validation retries surface their attempt count on the span. A report script aggregates per-role calls, errors, tokens, cache traffic, cost, and latency percentiles from the trace store — the before/after instrument every subsequent model or prompt change will be judged with. The docs now describe the system that exists.\
**Consequences:** A live turn now shows the orchestrator writing ~11k tokens to cache once and reading them back on every subsequent step at a tenth the price — and shows the location resolver paying for ~6k uncached tokens every single turn, which is the next fix, now with a number attached. Unpriced spend is visible as a gap (a model without a pricing entry reports no cost rather than a guessed one), and a test forces every configured role to be priceable so new roles can't reintroduce silence. Failure rates may look nominally higher on background paths since transient transport errors are no longer silently absorbed twice per call before anyone counts them; those paths fail open by design and their retry budgets are now a config decision rather than an SDK default. The pricing table joins the monthly reconciliation loop as live code, so a stale rate is now a wrong number on every span rather than a stale comment.
