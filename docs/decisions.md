...
## ADR-144: An answer is a set of relations, not a rendering — and the areas are the path

**Date:** 2026-08-06\
**Status:** accepted — follows ADR-142\
**Context:** Two problems, one root. First, a **regression ADR-142 introduced**: in each place tool the area-anchor branch returned before the corridor check, so a trip turn where the agent also named areas silently lost everything ADR-136 built — no off-route filtering, no route ordering, no length gate. It was visible and missed in that step's own verification, which returned a park in Hoi An for a Da Nang→Hue ride. Second, the **answer had no shape**: candidates arrive in per-tool payloads, so a saved place in Hoi An and a suggestion next door land in different `tool_results` entries with nothing tying them together. The product wants an ordered set of sections — the place, then what is at it, with the user's own saves mixed in — and that grouping can only be computed after every tool has run.\
**Decision:** **The agent decides whether there is a road.** `suggest_areas(travel_between: true)` makes the named areas an ordered path; without it they are independent places that happen to share an answer. This is not inferred from distance, because distance cannot separate the three cases the model separates effortlessly: Hoi An→Hue is a ride worth stopping along, two neighborhoods in one city have nothing between them, and Hanoi→Saigon is a flight. When a path exists, the stretches between consecutive areas are searched as well, gated **per leg** (ADR-138) rather than all-or-nothing like `route_too_long` — a trip with one flight and one drive keeps the drive's stops. The answer is then assembled once, at finalize, as **flat items plus an ordered group index**: every item appears exactly once carrying `group_key` and `route_progress`, and `groups` carries order and metadata only. Not a tree with items nested inside, because kebi has several clients and a tree fixes one grouping, duplicates items the moment a second view exists, and bakes item order into the contract. Kebi returns what only kebi can compute — which area a place was found at, whether the turn is a journey, where each item sits along it — and nothing about layout.\
**Consequences:** ADR-136's corridor is restored by *composition* rather than precedence: anchors own the geography when the agent named areas, and the resolver's corridor owns it otherwise. Streaming is unaffected — `tool_result` frames already arrive in one terminal batch, so `answer` rides along with them; progressive reveal is deferred, not designed away. Three costs are accepted. **Attribution favours the endpoints**: Hai Van Pass sits 18 km from Da Nang's centre and files under Da Nang rather than under the ride. Attributing by a tight radius instead was tried and broke worse — post-merger Vietnamese city rows carry *province* centroids, so Hue's Imperial City is 16 km from the stored "Hue" point and the city's own landmarks came back as "on the way". On a route that bends, neither the chord's progress nor the centroid's distance is trustworthy, and a stop filed under a real containing area is at least true. **An administrative boundary is not a search radius** — that same merger gave `vn/da-nang` a 160 km bbox, which uncapped became an 80 km disc that swallowed the whole ride; reach is now bounded per kind while the bbox still travels to the client as `extent`, because the boundary is true even when the radius would not be. And the grouped view is **additive**: everything in it is also in `tool_results`, so a client that ignores it loses nothing and a failure to assemble it costs the grouping, never the answer.

---

## ADR-142: An area is an answer and a search anchor — and a pass is an area, so it can never be saved as a venue

**Date:** 2026-08-06\
**Status:** accepted — location-kinds Step 6\
**Context:** Two problems the roadmap left open converge on the same missing idea. Consult could not put an **area** forward even when the area *was* the answer: "which neighborhood should I stay in?" got a hotel or a paragraph of prose, because the answer contract had no way to say "this item is a place the size of a district". And a validated non-venue could still be **saved as a venue** — Google holds two records for Hai Van Pass, one typed as natural geography (which ADR-133 correctly drops) and one typed `historical_landmark`, indistinguishable from any restaurant. A save-time guard was investigated and abandoned: the geocoder types the pass and Lang Co Beach identically, so any guard blocking the pass also blocks the beaches, lagoons and springs that are the best stops on that same road. Underneath both sat a third, larger failure that ADR-140 names: kebi would talk about Hue, Hoi An and the pass in prose and pin nothing at any of them, because every search was a disc around the turn's origin. An answer that names three places and grounds none of them is the answer a general assistant already gives.\
**Decision:** Areas become first-class in the answer, and — the part that matters more — **first-class as an input to retrieval**. Three things follow. (1) The entity store accepts every granularity a question can ask about: regions, neighborhoods, named natural features and named streets, resolved through **one engine with a declarative spec per kind**, so a new kind is a new spec rather than a new resolver and a new branch. Verification is unchanged and deliberately so: the match must round-trip against the name that was asked for, which is exactly what separates a street from a journey — Hanoi Train Street has a footprint the provider returns, Ha Giang Loop does not and still collapses to its containing area. This narrows the untrusted-routes rule to what it was written against: **bounded geography is an area; a named journey is not**, and the line is the footprint rather than the noun. (2) The agent names the areas worth going to and kebi **searches anchored on each one** — the user's saves there, taste-ranked venues, discovery for gaps, and the area's own claims. This is ADR-140 applied to areas, and it costs one tool call however many areas are named, because the fan-out happens inside the tool. (3) A candidate declares its `kind`, and an area candidate has no `place` — so it has no save-as-venue path at all. **The pass problem dissolves rather than being guarded**: keeping an area is a taste signal, not a row.\
**Consequences:** An area answer is renderable (a shaded extent, never a pin) and keepable, and the library stays venue-only — the demand-gated half is untouched. Three costs are accepted deliberately. **Extent is only as good as the provider**: Google returns a ~0.4 km box for a ~20 km road, so an implausible extent is not corrected but *disbelieved* — the area degrades to a point with a radius, because kebi has no better geometry for a linear feature and inventing one would draw a confident lie. **The kind correction is store-dependent**: a non-venue kebi has never resolved still comes back as a venue, so the guarantee strengthens as the store fills rather than holding on day one. **Area ranking is only as good as the claims**, which is Step 7's job; until then a "why this area" line leans on taste and the agent's own knowledge. The answer item shape (`id`, `kind`, `extent`) shipped here rather than in Step 5, so the app integrates once and Step 5's revision work needs no second contract change.
...
## ADR-143: Region interest is a fact of what the user named, so it rides its own event

**Date:** 2026-08-04\
**Status:** accepted — restores ADR-135's stated intent. *Renumbered 2026-08-06: this shipped as a second ADR-141, colliding with the moat ADR below; the moat kept 141 because the roadmap cites it by number.*\
...
**Date:** 2026-08-04\
**Status:** accepted — builds on ADR-136/137\
**Context:** ADR-137 made single-leg journeys good, and a multi-city question exposed a different failure. Asked about a trip through three cities, kebi narrated a continuous road trip with stops invented along a 548 km leg that essentially nobody drives, while the same question put to a general model opened by naming the flight and the overnight train, then covered what to do in each city. The gap was not knowledge or retrieval: the length gate that decides a leg is too long for stops was already computed and already suppressing sampling on that leg — it simply never reached the answer, so the agent was handed an instruction to narrate a drive and did. Everything else followed from that one omission: no transport reasoning, no time budgeting, and no depth at the places the trip was actually about, because the search was framed entirely as "what is between". Two card defects surfaced alongside it. A stop the agent named by its real name came back as a different business that merely sat closer to the search centre — the karst boat ride resolving to a guesthouse of the same name — because name validation took the nearest hit without ever checking it was the place asked for. And a place the user had saved shipped twice in one answer, once from the saved-places tool and once as a fresh suggestion, since the consult tools dedupe internally but never against each other.\
**Decision:** A multi-stop trip is treated as several trips at different scales rather than one journey. Each leg carries its own length and its own verdict on whether it is a drive at all, and that verdict reaches the answer. A leg too long for stops is no longer narrated as a drive: the agent says how people actually cover it — a flight, an overnight train, a sleeper bus, roughly how long and roughly what it costs, from its own knowledge — and then treats both of its endpoints as destinations worth describing in their own right, while the drivable legs keep the stop-by-stop treatment. This stays on the right side of the line the roadmap draws: it is judgement about a trip, not a booking service, so no timetables, no exact fares, and nothing persisted. The same reframing fixes the depth problem, because a trip through several places is mostly about those places: the agent now names what is worth doing *at* each named stop as well as between them, which costs nothing in retrieval since every named stop is already a sampled waypoint inside the bounds the search validates against. Separately, name validation gains the round-trip discipline the area path has had since ADR-126 and the venue path never adopted: a handful of provider results are considered instead of one, an exact name match wins outright, a name that merely wraps the asked-for one comes next, and otherwise the provider's own nearest-first order stands. It is deliberately a preference and not a filter — a strict name check would reject a place returned under its local-language name, which is the same place. Finally, a consult tool drops any place another tool has already surfaced in the same turn, so one place is one card; the saved copy wins, because a place the user has been meaning to visit is a better card than a suggestion.\
**Consequences:** A multi-city trip now reads as a trip — how to cover the long hops, what to do at each end, and real ordered stops on the stretches actually driven — with the cards matching what the prose says. Wrong cards get materially rarer: the correct records were already being returned by the provider and simply losing to proximity. The cost is a slightly larger provider response per name, which is not billed (result count is free; the field mask sets the tier). Two limits are accepted rather than solved. The scale verdict is a straight-line distance against a single configured threshold, so it will occasionally misjudge a leg that is genuinely driven or genuinely flown near the boundary; it is a config value precisely so it can move. And name matching cannot distinguish a landmark from a facility at that landmark when both wrap the asked-for name equally closely, so the shorter name wins as a heuristic rather than a guarantee. **A hole stays open and is now explicitly claimed rather than implied:** ADR-137 let a validated non-venue become a card, and a save cannot tell it apart from a real venue. The type signal that would have carried this does not exist — the provider holds two records for the same mountain pass, one typed as natural geography and correctly dropped, one typed as a historical landmark and indistinguishable from any other venue — so a type-based guard was investigated and abandoned rather than replaced with a name heuristic or a curated list. It needs its own decision.

---

## ADR-137: The agent suggests and kebi searches around it — knowledge is the spine of an answer, a card is what gets verified

**Date:** 2026-08-04\
**Status:** accepted — narrows ADR-133, builds on ADR-118/136\
**Context:** Live-testing ADR-136 exposed something the geometry could not fix. Asked for a trip between two cities, kebi returned one or two venues and apologised, while a plain language model answered with five real stops and the texture that makes them worth the drive — where to pull over, why the light is better early. Three causes, none of them retrieval. Every stop in that better answer *validates* in kebi's own pipeline, correctly placed and correctly ordered: they were simply never proposed, because naming was delegated to a small helper model while the orchestrator — the strongest model in the turn, and the one that actually knows the road — was forbidden from naming anything. The agent prompt made that explicit: names mentioned to the user had to come from a tool result, and timing, fee and safety tips had to come from stored notes "never from your own general knowledge". And the third cause is structural: a mountain pass has almost no venue data in any provider, because its value is the experience of being there, which no layer was permitted to voice. The rule doing the blocking was written against a real harm — a route saved to the library as a venue — but had grown to cover speech as well as storage.\
**Decision:** The agent's own knowledge of the world becomes a first-class source rather than a fallback, and retrieval serves its judgement instead of replacing it. **The agent suggests, and kebi searches around what it suggests**: it names the places it knows are worth going to and hands them to the propose-and-validate tool, which verifies them exactly as it verifies a helper model's names — an invented name is dropped either way — so knowledge and pins become one answer instead of two competing ones. The helper namer remains, as the fallback for when the agent has nothing specific in mind. An answer of this shape has three layers: the agent's knowledge as the spine, validated places as pins where they exist, and the user's own saved places along the route, which required the saved-place search to be fenced by the whole journey rather than a disc around its start. The honesty of the whole thing rests on one line: **prose may carry knowledge; a card must be earned.** Anything the user can tap, save, or see pinned comes from a tool result — which is precisely what ADR-133 was written to protect, and both of its original incidents stay closed, since neither saving a route as a venue nor returning one as a place card is a matter of prose. What the agent *says* is not so limited: it may name a pass, a bay, a stretch of coast and describe it from what it knows. One carve-out survives intact — operating facts, meaning hours, prices, phone numbers and the like, are stated only from a tool result, because they are checkable, they change, and a wrong one sends someone to a locked door. Stored local knowledge is still preferred where it exists; its absence is no longer a reason to go quiet.\
**Consequences:** Journey answers read like someone who has driven the road, while the stops that can be verified still arrive as real, mappable places, and a user's own saves surface where they fall along the way. The weaker naming model leaves the path whenever the agent already knows an area, which removes a call rather than adding one. The cost is a genuine loosening of a guarantee: kebi may now describe a place that has closed or misremember a detail, where before it would have said nothing. That trade was taken deliberately — silence was not neutral, it was useless to someone standing there deciding what to do — and it is bounded by keeping cards verified and operating facts tool-only. The durable answer to staleness is the knowledge layer (ADR-118/120): experience knowledge re-derived by a model every turn and thrown away should instead accrete as claims that can be corrected, which is the roadmap's area-knowledge work. Rollout is prompt and config only — no migration, no contract change, no cache invalidation, and the arg surface gains one optional field on a single tool, which is additive for the product repo.

---

## ADR-136: "On the way" is a spatial filter, not a routing product — the route is a shape the search takes, and the answer is an ordered list nobody stores

**Date:** 2026-08-04\
**Status:** accepted — builds on ADR-084/091/134\
**Context:** Kebi is a places engine, not a route solution: it surfaces places near a point, on the way between two, or anywhere the question points, and "on the way" is a spatial filter exactly like "near me". Routing belongs to maps apps; what is worth stopping at belongs here. But the corridor scope ADR-084 introduced never became a filter at all — it resolved a destination and then only shaped prose, while every place search still drew one disc around the origin. So a trip-shaped question was answered from the model's memory rather than from validated data, which is how a mountain pass came back as a place to visit. Steps 1–3 of the location-kinds roadmap had already built the preconditions: non-venues no longer validate, both ends of a route resolve through the persisted area store, and region interest is a taste signal in its own right.\
**Decision:** A route becomes real geometry that the existing place search consumes, and nothing more. It is an ordered chain — the turn's origin plus the stops the user named, in the order they said them — because a trip is often several hops, not one. The search samples points along that chain, weighted toward the longer stretches; every stop the user named is always searched, while interior sampling is what a budget caps, so a spending limit can never quietly drop a place the user asked about. Results are filtered by distance to the route and ordered by progress along it: that ordering is what turns a set of places into a journey, and it is computed, not narrated by a model. The two place tools share one geometry, differing only in how they spend their calls — the name-validating tool covers the whole route with a single coarse bound so its call count is unchanged, while the provider-driven tool fans out across the sampled points. The tool owns the geometry; the agent curates, choosing which stops survive and writing the answer. **Scale is where honesty is required:** venue stops mean something up to roughly a long day's drive, and across a country nothing is meaningfully "on the way" — the real stops there are cities, which consult cannot yet return. So an over-long stretch is not sampled, a trip with no answerable stretch returns a distinct outcome that spends no model or provider call at all, and the agent says plainly that the trip is city-scale and works out which stretch the user means. That outcome is an answer, not a failure. Straight-line sampling is deliberate and sufficient: road-shape routing would make this a routing product, which it is not. Endpoint identity is resolved most-specific-first — the area store, then a country-scoped lookup accepted only when it lands on a real settlement, then a locally-qualified lookup for a generic name like "the airport"; qualifying a destination by the *origin* city, as the previous single-destination path did, silently resolved a city at the far end of the country to an unrelated address beside the starting point. A journey is never persisted: it lives in the tool result and the agent's answer, and there is no route table.\
**Consequences:** The done-when holds — a trip-shaped question now returns real, validated stops in journey order rather than one famous landmark, and a multi-hop trip is answered across its stretches. Two behaviour changes are deliberate and worth watching. A route turn where nothing sits near the route now returns empty where it used to return places clustered at the origin: the honest empty is the point, since the old answer was answering a different question. And a country-length trip now declines to invent stops. Cost moves only on route turns: name validation is unchanged, and provider-driven discovery multiplies by the waypoint cap. The whole geometry is scale-agnostic on purpose — the roadmap's later step, where an area can itself win an answer, changes only *what* is searched at each sampled point, never how the route is sampled, filtered, or ordered; and the ordered list this produces is exactly the working set the revisable-answers step will operate on, with progress along the route as the key that "put a stop between these two" resolves against. Rollout is config-only: no migration, no API contract change, no cache invalidation. Checkpointed conversations carrying the older single-destination shape are coerced forward explicitly rather than tolerated, so a turn in flight survives the deploy.

---

## ADR-135: A shared area is its own taste signal — region interest and experience type, distinct from venue sentiment

**Date:** 2026-08-02\
**Status:** accepted — builds on ADR-134/077\
**Context:** ADR-134 turned a share's noted non-venues into verified area entities and let them anchor the knowledge harvest — but they still meant nothing to the *taste* model. Sharing a whole region or a scenic route trained taste identically to liking one restaurant: the only location the taste model knew was a by-product of where a saved *venue* happened to sit, and it had no notion of experience type at all. So a loud interest signal — a user who keeps sharing northern Vietnam, or road-trip content — was flattened into nothing, and later suggestions leaned on none of it. The roadmap's Step 3 closes this, with the visible half (rendering saved areas in the library) deliberately demand-gated and the background half (signals, priors) unconditional.\
**Decision:** A share's areas become their own taste signal, kept deliberately distinct from venue sentiment. Two positive-only signal classes join the model: **region interest** — one per area a share noted, resolved to the verified entity ADR-134 already produces (a route collapses to its containing area) — and **experience interest**, the experience-type vocabulary (scenic route, motorbike route, hiking, …) a route/region share stands for. Both fire automatically off the share, through the same background pass that already harvests it; there is no "save an area" action and no saved object, matching the demand-gated posture. Region interest is a fact of the share and survives even when the content harvest yields nothing; experience type is semantic content, so it rides the tags the harvest surfaces. The signals ride the existing interaction log and taste re-aggregation unchanged — an area's signal carries its entity key and its display name, so aggregation keys the region bucket without a second lookup — and they land in their **own top-level buckets**, never folded into the venue-derived location context. That separation is the whole point: the taste summary can now say "drawn to the Hoi An region" or "likes scenic-route trips" as first-class patterns, distinct from "favors restaurants". Because that summary already feeds both the agent prompt and the candidate namer, the signals act as a **soft geo/experience prior** with no new retrieval code — later open-ended suggestions lean toward the regions and experiences a user keeps sharing.\
**Consequences:** The done-when holds: a shared route now influences later suggestions in its region, and the model tells "interested in a region" apart from "liked a restaurant". The cost is one enum-values migration (two new interaction types; no table change, `signal_counts` is JSONB) and a modest per-share write of a few interaction rows on the existing background path — no external calls (the area resolution is a store hit after ADR-134's harvest). Deliberate deviations from the roadmap sketch, each grounded: **no explicit save action and no library rendering** — the region signal is driven by the share itself, and the user-visible half stays demand-gated; **soft prior only** — region/experience bias rides the taste summary the agent and namer already read, while *hard* extent-scoping ("scope search to my saved Vietnam's bounds") is deferred to Step 4/5, which rewrites the same geofence and shouldn't be touched twice; and **region interest is harvest-LLM-independent while experience specificity rides best-effort harvest output** (a share whose harvest fails still trains region interest, just not the experience type that turn — self-healing on re-share). Rollout is the enum migration on deploy; nothing else rolls over — no Redis invalidation (the taste-regen and namer prompts are state-specific, uncached), no cross-repo contract change. The area entities ADR-134 persisted are now doing work behind answers; Step 4's corridor geometry and Step 5's area answers build on the same store.

---

## ADR-134: One persistent notion of an area — entity store, Google-backed geocoding boundary, and harvest anchored to noted areas

**Date:** 2026-08-02\
**Status:** accepted — builds on ADR-126/133\
**Context:** Area knowledge was fragmented and ephemeral: four subsystems (the turn's location resolution, the home greeting, the knowledge writers, research) each geocoded independently and threw the result away — extents fetched and discarded, no shared identity for "Da Nang" across turns or tools, and every repeat lookup re-paid the geocoder. That geocoder was the free community instance, whose ~1 request-per-second ceiling cannot back production. And ADR-133's bleed-stopper exposed a real gap: the knowledge harvest is anchored to persisted places, so a share whose every place was a noted non-venue — a route video, a region roundup — produced no claims at all; before the fix it harvested only because a mislabeled venue wrongly anchored it. The roadmap's locked direction: accept-and-type areas, one persistent authority, external route names never trusted as entities.\
**Decision:** A persistent area entity store becomes the single notion of "an area". Its key is exactly the canonical geo key the knowledge layer already uses, so every existing claim attaches to its entity with zero migration; hierarchy lives beside the key, not inside it. The store holds identity and geometry only — canonical name, learned spelling aliases, country code, centroid, extent, the provider's feature classification — while rich experiential data stays in the knowledge layer, joined by the shared key. Resolution is one service: store first, geocode only on miss, and only accepted when the ADR-126 round-trip discipline verifies the match (the returned feature must *be* the asked-for name, in the asked-for country, at an area-level feature type — provinces included, since they classify at city scope); a refused name is never substituted. Every former direct-geocode call site — turn location, corridor destination, home greeting, harvester, curator, research — now goes through this boundary, so each unique area is geocoded once ever and every verified lookup enriches the shared store. The geocoding transport itself moves behind a provider-agnostic seam, with Google's Geocoding API (the cheapest tier, an order cheaper than the text-search alternative) replacing the rate-capped free instance in both directions; reverse lookups sit behind a coordinate-bucket cache. Provider terms shape the schema: the provider's place ID is storable indefinitely and is kept as the entity's provider identity, while coordinates are provider content — geometry carries its own age and a stale read re-geocodes through the stored ID, mirroring the places catalog's compliance cycle. The harvest gap closes by making noted non-venues first-class anchors: a noted name travels with the location context its share placed it in, resolves through the subject-vs-container rule (an area noted in its own right anchors itself; a route — whose detection subtype now says so — skips the doomed self-probe and collapses to its containing area), and the share's claims key to the resolved entities. A bounded experience-type vocabulary (scenic route, motorbike route, hiking, …) joins the shared claim-tag list so a route's interest signal survives as tagged area knowledge. The expert curator stops free-texting area strings and names structured components instead, resolved through the same service. Consult stays venue-only; nothing surfaces area entities to any response contract in this step.\
**Consequences:** "Which area is this?" is now answered from one durable store instead of four throwaway lookups: repeat turns about a known city cost zero geocode calls, research lookups seed the store, and the Vietnam-video shape finally yields knowledge — three noted names become three verified area entities with real extents, the route's facts landing on its containing area tagged with experience types, no venue row anywhere. Steady-state geocoding spend rounds to zero (store + cache absorb nearly everything; the per-call rate sits in the cheapest tier with a free monthly allowance), recorded per call like every external cost. Deliberate deviations from the roadmap sketch, each grounded: no kind column on places (areas live in their own table; venues stay venue-shaped — revisit when library rendering needs a kind), settlement-density coarsens to a neutral default (the new provider carries no city/town/village signal; radius tiers still work), and a corridor destination that is not an area falls back to one coords-only lookup that persists nothing (airports must keep working; free-text stays banned from entity creation). Verified-or-refuse now also means a misspelled city asks for clarification where the old free-text lookup silently guessed — the intended tightening. Operational prerequisite: the Geocoding API must be enabled on the existing Google project before deploy; nothing else rolls over (no Redis invalidation, no contract change). The store's entities are the anchor Step 3's signals and geo priors, Step 4's corridor geometry, and Step 5's area answers build on.

---

## ADR-133: Non-venue geography is rejected at validation and narrated as interest, never saved

**Date:** 2026-07-22\
**Status:** accepted — extends ADR-124\
**Context:** Non-venues kept entering the catalog disguised as venues through both doors. The place provider co-stamps its generic attraction marker on routes and natural features, and that generic marker was enough to map a venue category — so a shared Vietnam video saved a 350 km motorbike loop as a landmark venue, and a trip-shaped query proposed a mountain pass as a place. The validation gate ADR-124 built only rejected administrative areas with *no* mapped category, so anything the provider flagged as an attraction sailed through; the extraction picker and the agent's candidate namer had no rule against route or region names at all; a rejected candidate simply vanished, violating the principle that a user action always has a visible consequence; and the library already held mislabeled rows. The location-kinds roadmap decided the direction: accept-and-type areas as first-class later, stop the bleeding now — with the detection written to become that later routing, not throwaway code.\
**Decision:** The validator's type-signal rule (ADR-124's single source of truth — never a name heuristic) is widened and named as an explicit decision point: a result whose types include any *specific* venue classification stays a venue; linear or natural geography is non-venue even when the generic attraction marker is present; administrative areas keep the district-as-attraction carve-out. Famous natural attractions with no specific venue type are deliberately on the non-venue side for now — a mountain stored as a point venue is precisely the mislabel — until the area model gives them a true home. Both LLM layers gain the matching rule: the extraction picker rejects route/area/region names as venue candidates (structurally marked as non-venue rejections, distinct from wrong-place rejections, and never substituting a similarly-named nearby business), and the candidate namer never proposes geography, offering visitable stops for journey-shaped intents instead. Rejection is narrated, not silent: a detected non-venue surfaces on the extraction envelope as a noted interest — a name plus a ready-to-render acknowledgment — and a share containing *only* non-venue geography is now a legal success outcome with no saves rather than a failure. Nothing is persisted for a noted interest in this step; the acknowledgment is the whole outcome. Existing mislabeled rows are deleted, not migrated: a one-off, dry-run-first cleanup re-verifies each suspect against the provider's live type signal with the same shared rule, and removes the saves, embeddings, the tainted taste-signal rows, and the catalog rows together.\
**Consequences:** The two incident shapes are closed at the cheapest layer — validation — with the LLM rules as defense in depth for rows persisted before the fix. The cross-repo contract gains one additive field and one new legal shape (a completed extraction with no results), which the product must render so the acknowledgment reaches the user. Known limits, accepted: the URL result cache serves only saved results, so a repeat share of a mixed URL omits the acknowledgments; and a legitimate venue the provider types only as generic geography would be wrongly rejected — none is known, and the carve-outs bound the risk. The named detection point is the seam Step 2 of the roadmap turns into venue-versus-area routing, at which point rejected geography stops being merely acknowledged and starts resolving to typed areas.
