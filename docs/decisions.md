...
**Date:** 2026-05-25\
**Status:** accepted\
**Context:** `ExtractionStatusRepository` was introduced (ADR-038, ADR-063) to back a `GET /v1/extraction/{request_id}` polling route reserved for a future async/background extraction variant. That variant was never built; the canonical extraction path (`POST /v1/extract`) has been synchronous from day one — the response carries the terminal envelope inline, and no client polls. The repository continued to be written by the service after every run (one extra Redis round-trip per extraction) and the route continued to ship as dead code, kept "in case we need it later." ADR-075 set a precedent for the opposite stance: prune dormant code rather than carry it. The cost of keeping it is small but real — one extra Redis write per save-path call, an extra service-constructor parameter, an extra deps factory, and a public route the product repo could mistakenly start depending on, which would lock us into a contract we never intended to support.\
**Decision:** Delete the status repository and the polling route. `ExtractionService` no longer takes a `status_repo` and no longer serializes the response post-run; it returns the envelope and is done. `GET /v1/extraction/{request_id}` is removed from the API contract. If an async variant is later genuinely needed, it will be designed against the requirements at that time — almost certainly with a job queue and a typed job-status store, not a Redis JSON blob keyed by request_id. `ExtractionResultCache` (ADR-074) is unrelated and unchanged — it indexes by canonical URL for cross-user reuse, a different access pattern.\
**Consequences:** One fewer write on the save path (small latency win, slightly less Redis chatter). One fewer constructor argument, one fewer deps factory, one fewer route, and the legacy `extraction:v1:*` / `extraction:v2:*` Redis key prefixes age out on their existing TTLs with no cleanup required. The product repo's contract narrows — the only extraction endpoint is now `POST /v1/extract`. Supersedes the polling-route portion of ADR-038 and ADR-063; the rest of ADR-063 (response envelope shape, `status` / `failure_reason` semantics) stands. Code-reviewers should be willing to apply the same logic to any other dormant scaffolding that has been carried "for a future variant" past one release cycle without a consumer.

---

## ADR-093: Extraction evidence — append-only object-storage ledger, not on the wire

**Date:** 2026-05-25\
**Status:** accepted\
**Context:** The `POST /v1/extract` response carried an `evidence: list[EvidenceDTO]` on every `ExtractPlaceItem` — the producer/medium audit trail of which enrichers contributed snippets to each picked place. The product repo never read it; it just inflated payloads and leaked internal pipeline shape across the repo boundary. At the same time the audit trail was genuinely useful — the same place can show up across many posts and videos over time, and the accumulated evidence is the signal that lets us reason about how a place is discovered and how often. Two valid storage targets: (a) per-row JSONB column on `places` or `user_places`, or (b) an object-storage ledger keyed by place. (a) buys SQL queryability for free but couples evidence growth to DB write amplification and forces a single-key read-modify-write under concurrent extractions of the same URL; (b) gives an append-only log with no concurrency hazard at the cost of needing list-by-prefix to read.\
**Decision:** Evidence is removed from the HTTP response and writes to an append-only object-storage ledger instead. Key scheme: `evidence/{place_id}/{iso8601-utc}-{request_id}.json`, one object per (place, extraction event). Each write is a new key, so two concurrent extractions of the same URL can't lose each other's writes — no read-modify-write. The bucket is reached via a new `ObjectStorageProtocol` (Protocol-based), with one S3-compatible adapter that covers Railway Object Storage today and AWS S3 / Cloudflare R2 / MinIO tomorrow — provider swap is endpoint-URL-only, no code change. A `NullObjectStorage` no-op fallback lets local dev run without bucket env vars. The writer is wired into `ExtractionService` and runs per persisted candidate; storage failures log and continue — the bucket is an out-of-band audit trail, not a save-path dependency. Cache hits (ADR-074) do not re-write evidence: the same canonical URL means the same evidence content, and the first save already wrote it. The product-repo contract narrows: `ExtractPlaceItem` is now `{place, confidence}` only.\
**Consequences:** The product repo's `/v1/extract` payload shrinks and stops leaking pipeline internals. Evidence accumulates over time per place — listing by prefix `evidence/{place_id}/` reconstructs the full ledger across every extraction that touched that place. Provider swap is one env var (`BUCKET_ENDPOINT_URL`) plus credentials; the Protocol means callers never type-depend on Railway specifically. Reading the ledger requires `list_prefix` (no SQL `WHERE place_id =` query) — acceptable today because no live consumer reads evidence; first consumer that needs it ships a small reader on top of the same Protocol. The bucket is non-critical: outages don't break extraction. Per-key overhead means high cardinality for very-popular URLs, but the cache deduplicates same-URL extractions at the first hop, so growth is bounded by distinct URLs touching a place, not by save count. Costs are not tracked in Langfuse (ADR-092) — object-storage spend is monitored at the provider level. The `EvidenceDTO` type and the response-side evidence field are deleted; tests that asserted on response shape were narrowed accordingly. Supersedes the response-shape part of ADR-063 (status was already removed by ADR-071; evidence is the last per-item field beyond `place + confidence` to go).
